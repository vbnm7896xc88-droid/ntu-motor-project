...
ARTIFACT_FILES = [
    "lstm_attention_best_search.pth",
    "feature_scaler.pkl",
    "target_scaler.pkl"
]
...
def find_artifact_dir():
    ...
    for folder in candidate_dirs:
        if all((folder / file_name).exists() for file_name in ARTIFACT_FILES):
            return folder

    return None

BASE_DIR = find_artifact_dir()

if BASE_DIR is None:
    st.error(
        "模型檔案載入失敗：找不到必要檔案。\n\n"
        "請確認以下三個檔案有放在 GitHub 專案根目錄，或全部放在 data 資料夾內：\n\n"
        "1. lstm_attention_best_search.pth\n"
...
def process_batch_input(df):
    ...
    if "Time_Step" not in df_processed.columns:
        if "Time" in df_processed.columns:
            df_processed["Time_Step"] = df_processed["Time"]
        else:
    ...
    for col in req_cols:
        df_processed[col] = df_processed[col].rolling(
...
st.markdown(
    "請選擇「實時單筆預測」或是「批次檔案上傳」來評估馬達的剩餘使用壽命 "
    "(Remaining Useful Life, RUL) 與健康指數。"
)
st.markdown("---")
...
with tab1:
    ...
    if st.button("啟動 單筆預測核心 (Run Prediction)", use_container_width=True):
        with st.spinner("推論中..."):
            ...
            try:
                # 若轉速為 0 或小於 0，直接判定剩餘壽命為 0
                if in_rpm <= 0.0:
                    final_rul = 0.0
                else:
                    input_tensor = process_single_input(
                        in_rpm,
                        in_vm,
                        in_ch1,
                        in_ch2,
                ...
                health_pct = min(
                    100.0,
                    max(0.0, (final_rul / NOMINAL_LIFESPAN) * 100)
                )
                ...
                with res_right:
                    ...
                    text_color = "black" if 20 < health_pct <= 50 else "white"

                    health_html = f"""
                    <div style="background-color: #f8f9fa; border-radius: 8px; width: 100%; height: 40px; margin-top: 15px; border: 1px solid #ced4da;">
                        <div style="background-color: {bar_color}; width: {health_pct}%; height: 100%; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: {text_color}; font-weight: bold; font-size: 18px; transition: width 0.5s ease-in-out;">
                            {int(health_pct)}%
                        </div>
                    </div>
                    <div style="margin-top: 10px; font-size: 18px; font-weight: 500;">
                        狀態：<span style="color: {bar_color}; font-weight: bold;">{status_text}</span>
                    </div>
                    """
...
with tab2:
    ...
    st.markdown(
        "支援 CSV 或 Excel (xlsx) 格式。檔案必須包含 "
        "`RPM(轉速)`, `Vm(電壓)`, `CH1(驅動器溫度)`, "
        "`CH2(馬達溫度)`, `dB(音量)`, `Im(電流)` 等核心特徵欄位。"
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            preview_cols = [
                ...
                if not str(c).endswith("_std") and c != "時間序列"
            ]
            ...
            if st.button("開始批次趨勢預測", use_container_width=True):
                with st.spinner("模型分析中，請稍候..."):
                    ...
                    if result[0] is None:
                        ...
                        # 避免長度不一致
                        full_predictions = full_predictions[:len(df_upload)]
                        ...
                        health_pcts = np.clip(
                            (df_upload["預測 RUL (s)"].values / NOMINAL_LIFESPAN) * 100,
                            0,
                            100
                        )

                        df_upload["健康等級"] = [
                            assign_status(p) for p in health_pcts
                        ]
                        ...
                        last_health_pct = min(
                            100.0,
                            max(0.0, (last_rul / NOMINAL_LIFESPAN) * 100)
                        )
                        ...
                        chart_data["時間(s)"] = df_upload["Time_Step"].values
                        ...
                        # 預測 RUL：紅色
                        chart_data["預測 RUL"] = df_upload["預測 RUL (s)"].values

                        df_melt = chart_data.melt(
                            id_vars="時間(s)",
                            var_name="指標",
                            value_name="剩餘使用壽命(s)"
                        )

                        chart = alt.Chart(df_melt).mark_line(size=2.5).encode(
